...
<template>
  <div ref="scroll_element" class="lg:pt-14">
    <NuxtLink :to="app_routes.messages.chat(chat.id)" v-for="chat in chats" :key="chat.id" class="flex items-center space-x-4 mb-4 bg-base-white rounded-lg py-4 px-3">
      <div class="w-10 shrink-0">
        <NuxtImg :src="chat.sender.img" class="avatar h-10 w-10" />
      </div>
      <div>
        <div class="text-main">{{ chat.sender.name }}</div>
        <div class="truncate w-11/12 text-muted text-ellipsis overflow-x-hidden">{{ chat.text }}</div>
      </div>
    </NuxtLink>
  </div>
</template>
