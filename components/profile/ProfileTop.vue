...
const img = useImage();
const isSameUser = computed(() => authStore.user.id === props.user.id);
  ...
  <div>
    ...
    <div class="flex items-center justify-between py-3">
      <NuxtImg
        ...
        :src="props.user?.img"
        :alt="props.user.name"
        :placeholder="img(props.user?.img as string, { h: 70, w: 70, f: 'png', blur: 2, q: 50 })" />
      ...
      <button v-else class="bg-purple-700 text-white py-2 px-6 rounded-3xl font-medium capitalize">{{ t("profile.follow") }}</button>
    </div>
    ...
    <p class="text-sm text-gray-600 py-4" v-if="props.user.bio">{{ props.user.bio }}</p>
  </div>
</template>
