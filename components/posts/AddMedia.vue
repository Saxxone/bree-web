  ...
  <div>
    <div v-if="fileList" class="overflow-x-scroll space-x-4">
      <div class="flex w-fit items-center space-x-4 pr-4 pb-3">
        <div v-for="(file, index) in fileList" :key="file.name" class="w-48 rounded-lg overflow-hidden relative h-56">
          <div
          ...
          <video v-if="file.type.includes('video')" :src="createObjectURL(file)" class="w-full h-full object-cover" :alt="file.name" />
        </div>
      </div>
    </div>
    <div class="flex items-center pt-2 pb-2">
      <div v-for="(item, index) in attachments" :key="item.icon" class="cursor-pointer" @click="attachments[index].command()">
        <span class="material-symbols-rounded dark:text-gray-300 text-2xl">{{ item.icon }}</span>
      </div>
    </div>
  </div>
</template>
